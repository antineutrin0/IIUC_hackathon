...
const SuggestedJobsPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const jobsRes = await axios.get(
          "https://iiuc-hackathon-backend.vercel.app/jobs",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  const calculateMatchScore = (job, profile) => {
    ...
    if (
      userRoles.some(
        (role) => jobTitle.includes(role) || jobTrack.includes(role),
      )
    ) {
      score += 15;
  ...
  const excellentMatches = filteredJobs.filter(
    (j) => j.matchScore >= 70,
  ).length;
  const goodMatches = filteredJobs.filter(
    (j) => j.matchScore >= 50 && j.matchScore < 70,
  ).length;
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 via-blue-50 to-green-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-medium">
            Analyzing your profile and finding perfect matches...
          </p>
        </div>
      </div>
    );
          ...
          <div className="bg-white p-12 rounded-xl text-center shadow-md">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-400 text-sm">
                Try updating your profile with more skills or adjust your search
                criteria
              </p>
            </div>
          </div>
        ...
        {pageCount > 1 && (
          <div className="flex justify-center mt-10">
            <ReactPaginate
              ...
              pageClassName={
                "px-5 py-3 bg-white border border-gray-300 rounded-lg hover:bg-green-50 hover:border-green-300 transition-colors font-medium"
              }
              previousClassName={
                "px-5 py-3 bg-white border border-gray-300 rounded-lg hover:bg-green-50 hover:border-green-300 transition-colors font-medium"
              }
